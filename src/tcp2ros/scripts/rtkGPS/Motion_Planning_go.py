...
class Motion_Planning:
    def __init__(self,speedXY=0.2,speedYaw=0.2, errorMeter=0.2, errorAngle=math.pi/6.0):
        ...
        print "Motion_Planning_go start"
    ...
    def reach_position(self):
        ...
        if abs(self.target[0] - self.position[0]) <= self.error_meter and math.sqrt((self.target[0] - self.position[0])*(self.target[0] - self.position[0])+(self.target[1] - self.position[1])*(self.target[1] - self.position[1]))<self.error_meter:
            return True
        else:
            return False
    ...
    def MotionPlan(self):
        ...
        if self.target and self.position :
            if  math.sqrt((self.target[0] - self.position[0])*(self.target[0] - self.position[0])+(self.target[1] - self.position[1])*(self.target[1] - self.position[1]))> self.error_meter:
                if self.target[0]>0:
                    tempAngle = math.atan(self.target[1]/self.target[0])
                    if(abs(tempAngle) > 15*math.pi/180):
                        if tempAngle > 0:
