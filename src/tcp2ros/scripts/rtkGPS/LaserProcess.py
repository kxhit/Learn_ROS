...
class LaserProcess:
    def __init__(self):
        ...
        self.reach_y = 0
        self.reach_th = 0#rad
        self.reflact1_i_start = 0
        self.reflact1_i_end = 0
        self.reflact2_i_start = 0
        self.reflact2_i_end = 0
    ...
    def callback1(self, data):
                ...
                if i_state is True:
                    ...
                    if i_num>10:#find~
                        ...
                        if reflact_num_find == 0:
                            self.reflact1_i_start = i_start
                            self.reflact1_i_end = i_end
                            self.reflact1_x = x_temp
                            self.reflact1_y = y_temp
                            reflact_num_find = reflact_num_find + 1
                        else:
                            self.reflact2_i_start = i_start
                            self.reflact2_i_end = i_end
        ...
        #print self.res[1], self.res[0], self.res[2]/math.pi*18
        
        if reflact_num_find is 2 and temp_length>0.5 and temp_length<1.5:
        ...
        stop_flag = False
        for i in range(0, len(data.ranges)):
            if data.ranges[i] < self.stop_radius and data.ranges[i] > 0.01 and i*data.angle_increment>math.pi/6 and i*data.angle_increment<math.pi*5/6:
                if i >= self.reflact1_i_start and i <= self.reflact1_i_end:
                    continue
                if i >= self.reflact2_i_start and i <= self.reflact2_i_end:
                    continue
                stop_flag = True
